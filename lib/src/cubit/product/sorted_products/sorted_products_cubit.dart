...
class SortedProductsCubit extends Cubit<SortedProductsState> {
  ...
  Future<void> fetchProductsFromCategory({required String category}) async {
    emit(SortedProductsLoading());

    try {
      // Assume you have a ProductApiProvider instance
      final apiProvider = GetProductsFromCategory();
      final products =
          await apiProvider.getProductsFromCategory(category: category);
